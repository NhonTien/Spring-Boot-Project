...
function searchProductsWeb(pagerNumber) {
	var data = {
		...
		"categorySlug": $("#categoryId").val(),
		"status": '1'
	};
	$(".messSearch").addClass('d-none');
	$.ajax({
		...
		success: function(responseData) {
			if (responseData.responseCode == 100) {
				if(responseData.data.productListWeb == 0){
					$(".messSearch").empty();
					var mess = '<h6>Không tìm thấy sản phẩm phù hợp</h6>'
					$(".messSearch").append(mess);
					$(".messSearch").removeClass('d-none');
...
function renderProducts(productListWeb) {
	...
	$.each(productListWeb, function(key, value) {
		rowHtmlss = '<div class="card" id="myTable">'
			...
			+ '</div>'
			+ '<div class="card-body">'
			+ '<p class="card-text">Giá: ' + formatNumber(value.price*(1 - value.discount/100)) + '<sup><u>đ</u></sup></p></p>'
			...
			+ '<a class="btn btn-primary cart" data-id="' + value.productId + '" href="/cart/add/' + value.productId + '"><i class="fas fa-shopping-cart"></i></a>'
			+ '</div>';
