		...
		$.ajax({
			...
			success: function(responseData) {
				if (responseData.responseCode == 100) {
					...
					$('#addresss').val("Địa chỉ giao hàng:		" + orderInfo[0].order.aptSuit + ", " + orderInfo[0].order.ward.name + ", " + orderInfo[0].order.district.name + ", " + orderInfo[0].order.province.name);
...
function searchOrders(pagerNumber) {
	$.ajax({
		...
		success: function(data) {
			...
			var x = $('#userId').val();
			$.ajax({
				url: "/admin/order/api/findAll/" + x + "/" + pagerNumber,
				type: 'GET',
				dataType: 'json',
				contentType: 'application/json',
				success: function(responseData) {
					if (responseData.responseCode == 100) {
						renderOrdersTable(responseData.data.ordersList);
